...
class TrainDataLoader(object):
    ...
    def __init__(self):
        ...
        data_file = './data/ASSIST17/train_set.json'
    ...
    def next_batch(self):
        ...
        input_stu_ids, input_exer_ids, input_knowedge_embs, ys, affects = [], [], [], [],[]
        for count in range(self.batch_size):
            ...
            affects.append(log['affect'])
            input_knowedge_embs.append(knowledge_emb)
            ys.append(y)

        self.ptr += self.batch_size
        return torch.LongTensor(input_stu_ids), torch.LongTensor(input_exer_ids), torch.Tensor(input_knowedge_embs), torch.LongTensor(ys), torch.tensor(affects)
...
class ValTestDataLoader(object):
    def __init__(self, d_type='validation'):
        ...
        if d_type == 'validation':
            data_file = './data/ASSIST17/test_set.json'
        else:
            data_file = './data/ASSIST17/test_set.json'
        config_file = 'config.txt'
        with open(data_file, encoding='utf8') as i_f:
    ...
    def next_batch(self):
        ...
        input_stu_ids, input_exer_ids, input_knowledge_embs, ys,affects = [], [], [], [],[]
        for log in logs:
            ...
            ys.append(y)
        self.ptr += 1
        return torch.LongTensor(input_stu_ids), torch.LongTensor(input_exer_ids), torch.Tensor(input_knowledge_embs), torch.LongTensor(ys), torch.tensor(affects)
